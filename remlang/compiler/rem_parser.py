...
symbol = AstParser([LiteralParser('[a-zA-Z\u4e00-\u9fa5\u3040-\u309f\u30a0-\u30ff]{1}[a-zA-Z\u4e00-\u9fa5\u3040-\u309f\u30a0-\u30ff\d_]*', name='\'[a-zA-Z\u4e00-\u9fa5\u3040-\u309f\u30a0-\u30ff]{1}[a-zA-Z\u4e00-\u9fa5\u3040-\u309f\u30a0-\u30ff\d_]*\'', isRegex = True)], name = 'symbol')
...
lambdef = AstParser([LiteralParser('{', name='\'{\''),SeqParser([LiteralParser('|', name='\'|\''),SeqParser([Ref('symbol'),SeqParser([LiteralParser(',', name='\',\''),Ref('symbol')])], atmost = 1),LiteralParser('|', name='\'|\'')], atmost = 1),SeqParser([Ref('T')], atmost = 1),SeqParser([Ref('statements'),SeqParser([Ref('T')], atmost = 1)], atmost = 1),LiteralParser('}', name='\'}\'')], name = 'lambdef', toIgnore = [{"T"},{'|','}','{',','}])
...
trailer = AstParser([LiteralParser('[', name='\'[\''),Ref('exprMany'),LiteralParser(']', name='\']\'')], name = 'trailer', toIgnore = [{},{'[',']'}])
atomExpr = AstParser([Ref('atom'),SeqParser([LiteralParser('!', name='\'!\''),SeqParser([Ref('trailer')])], atmost = 1)], name = 'atomExpr', toIgnore = [{},{'!'}])
...
statement = AstParser([DependentAstParser([Ref('expr')],[Ref('let')],[Ref('breakUntil')]),SeqParser([LiteralParser(';', name='\';\'')], atmost = 1)], name = 'statement')
let = AstParser([LiteralParser('`let`', name='\'`let`\''),Ref('symbol'),LiteralParser('=', name='\'=\''),Ref('expr')], name = 'let', toIgnore = [{},{'`let`','='}])
exprMany = AstParser([Ref('expr'),SeqParser([SeqParser([SeqParser([Ref('T')], atmost = 1),LiteralParser(',', name='\',\''),SeqParser([Ref('T')], atmost = 1),Ref('expr')], atleast = 1),SeqParser([Ref('T')], atmost = 1),SeqParser([LiteralParser(',', name='\',\'')], atmost = 1)], atmost = 1)], name = 'exprMany', toIgnore = [{"T"},{','}])
unpack = AstParser([LiteralParser('...', name='\'...\''),Ref('expr')], name = 'unpack', toIgnore = [{},{'...'}])
exprCons = AstParser([Ref('exprMany'),SeqParser([Ref('unpack'),SeqParser([LiteralParser(',', name='\',\''),Ref('exprMany')], atmost = 1)]),SeqParser([LiteralParser(',', name='\',\'')], atmost = 1)], name = 'exprCons', toIgnore = [{},{','}])
kv = AstParser([Ref('expr'),LiteralParser(':', name='\':\''),Ref('expr')], name = 'kv', toIgnore = [{"T"},{':'}])
kvMany = AstParser([Ref('kv'),SeqParser([SeqParser([SeqParser([Ref('T')], atmost = 1),LiteralParser(',', name='\',\''),SeqParser([Ref('T')], atmost = 1),Ref('kv')], atleast = 1),SeqParser([Ref('T')], atmost = 1),SeqParser([LiteralParser(',', name='\',\'')], atmost = 1)], atmost = 1)], name = 'kvMany', toIgnore = [{"T"},{','}])
kvCons = AstParser([Ref('kvMany'),SeqParser([Ref('unpack'),SeqParser([LiteralParser(',', name='\',\''),Ref('kvMany')], atmost = 1)]),SeqParser([LiteralParser(',', name='\',\'')], atmost = 1)], name = 'kvCons', toIgnore = [{},{','}])
...
breakUntil = AstParser([LiteralParser('`break-until`', name='\'`break-until`\''),Ref('symbol')], name = 'breakUntil', toIgnore = [{},{'`break-until`'}])
...
breakUntil.compile(namespace, recurSearcher)
